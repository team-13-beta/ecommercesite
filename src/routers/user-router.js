...
userRouter.get(
  ...
  async function (req, res) {
    ...
    if (!user) {
      // 해당계정이 존재하지 않으므로 auth 사용자 회원가입 창으로 리다이렉트
      req.session.info = {
...
userRouter.get("/logout", async (req, res, next) => {
  await req.session.destroy((err) => {
    if (err) {
      // 에러가 발생하면 세션에 상태를 저장 -> 이후에 session DB를 훑어서 삭제.
...
userRouter.get("/userlist", async function (req, res, next) {
  try {
    ...
    let option = "";
    if (req.currentRole !== "admin") option = req.currentUserId;
...
userRouter.get(
  ...
  async function (req, res, next) {
    try {
      ...
      // 사용자 정보를 업데이트함.
      const updatedUserInfo = await userService.updateUserBySession(
        userId,
        toUpdate,
      );
...
userRouter.get("/:user_id", loginRequired, async (req, res, next) => {
  try {
    ...
    if (req.currentRole !== "admin")
      throw new Error("일반 사용자는 접근권한이 없습니다!");
